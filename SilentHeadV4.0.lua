...
function HOME()
    MultiMenu = gg.choice({"💥Silent Aim💥", "🏙TP To DmZone🏙", "🔫GUN FOR WAR🔫", "🚫EXIT"},nil,"🔱UserName: "..username.."\n  🔰now : "..os.date("%Y").."/"..os.date("%m").."/"..os.date("%d").. "\n 🔰Expired in : 2022/10/30\n\n🇮🇷[S]amp [C]heats\n 💯creator : @No_Pulse\n 🔰Channel : @Cheaters_Samp")
      ...
      if MultiMenu == 3 then
        guninput = gg.prompt({
            "Tedad Gun Haye Khod Ra Vared Konid"},
                     {nil},
                                 {'text'})
    
        tedadD = guninput[1]
        gg.alert("⭕Be Game Rafte Va Chand Tir Bezanid VA Sepas Dokme OK Ro Bezanid Va Tedad Gum Baghi Mande Ra Vared Konid")
   ...
   function silentBody()
    ...
    function gunwar(tedadDGL, refineNUMB)
        gg.setVisible(false)
        gg.processResume()
        gg.searchNumber(tostring(tedadDGL), gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1, 0)
        gg.processResume()
        gg.refineNumber(tostring(refineNUMB), gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1, 0)
         = gg.getResults(100, nil, nil, nil, nil, nil, nil, nil, nil)
        gg.processResume()
        gg.editAll("9999", gg.TYPE_DWORD)
        for i, v in ipairs(srch) do
        v.freeze = true
        end
        gg.toast("Cheat Active✅")
        gg.clearResults()
